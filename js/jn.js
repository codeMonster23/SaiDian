// JavaScript Document
function allChecked(selectAllClassName, checkboxClassName){
//The first Parameter is the select all checkbok class name, the second Parameter is the checkbox class name which you what to check.
//第一个参数是全选按钮的class的名称，第二个参数是希望这个全选按钮选中的checkbox的class名称。
    if($("."+selectAllClassName+"").attr("checked") == "checked") {
        $("."+checkboxClassName+"").attr("checked", true);
    }else{
        $("."+checkboxClassName+"").attr("checked", false);
    }
}
...
$(function () {
    $(".upload-pic .btn-mstj").hide();
    $(".upload-pic").mouseover(function(){
        $(this).find("a").show();
    }).mouseout(function(){
        $(this).find("a").hide();
    })

    $(".mid-box .delete-icon").hide();
